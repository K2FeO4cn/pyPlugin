...
class PluginHookManager:
    def __init__(self) -> None:
        self.hooktable: dict[str, list[dict[str,
                                            Callable[[Any, Any], Any] | int]]] = {}
    ...
    def create(self, hook_name: str = None, default_handler: Callable[[Any, Any], Any] = None) -> None:
        ...
        if default_handler is not None:
            self.hooktable[hook_name].append(
                {"function": default_handler, "priority": 0})
        self.logger.debug(
            "Creating hook \"%s\" had successfully ended.", hook_name)
        return
    ...
    def publish(self, hook_name: str = None, args: tuple = None):
        ...
        for subscriber in self.hooktable[hook_name]:
            try:
                ...
                return
        self.logger.debug(
            "Publishing hook \"%s\" had successfully ended.", hook_name)
        return
    ...
    def register(self, hook_name: str = None, func: Callable[[Any, Any], Any] = None, priority: int = 1):
        ...
        while True:
            ...
            if priority >= self.hooktable[hook_name][i]['priority']:
                self.hooktable[hook_name] = self.hooktable[hook_name][:i] + [
                    {"function": func, "priority": priority}] + self.hooktable[hook_name][i:]
                break
            i += 1
        self.logger.debug(
            "Registering hook \"%s\" had successfully ended.", hook_name)
        return
    ...
    def init(self) -> None:
        try:
            filehandler = logging.FileHandler(
                filename=self.filename, mode="w+", encoding="UTF-8")
            logging.basicConfig(handlers=[logging.StreamHandler(), filehandler], force=True,
                                level=self.level, format='[%(levelname)s][%(asctime)s][%(name)s]%(message)s')
        except Exception as e:
...
class ConfigManager:
    ...
    def init(self) -> None:
        if not os.path.exists(self.file):
            self.logger.warning(
                "Config file \"%s\" doesn't exist! Creating...", self.file)
    ...
    def save(self) -> None:
        try:
            ...
            self.logger.error(
                "Failed to save config file. Trying reinitialize the file.")
            self.init()
    ...
    def load(self) -> None:
        try:
            ...
            self.logger.error(
                "Failed to load config file. Trying reinitialize the file.")
            self.init()
...
def initPlugins():
        ...
        if plugin_list[i].endswith(".py"):
            ...
            while True:
                ...
                if k == len(plugin_priored):
                    plugin_priored.append(
                        {"name": plugin_list[i], "priority": priority})
                    break
                if priority >= plugin_priored[k]['priority']:
                    plugin_priored = plugin_priored[:k] + [
                        {"name": plugin_list[i], "priority":priority}] + plugin_priored[k:]
                    break
                k += 1
        else:
    ...
    for i in range(len(plugin_priored)):
        if data["config"]["plugins"][plugin_priored[i]["name"]] > 0:
            logger.info("trying to load plugin \"%s\"...",
                        plugin_priored[i]["name"][:plugin_priored[i]["name"].find(".py")])
            try:
                plugin_object = importlib.import_module(
                    '.'+plugin_priored[i]["name"][:plugin_priored[i]["name"].find(".py")], package='plugins')
                ...
                logger.debug(
                    "failed to load plugin using importlib, trying exec")
                try:
                    plugin_global = {}
                    plugin_fp = open(
                        "./plugins/"+plugin_priored[i]["name"], "r", encoding="UTF-8")
                    ...
                    logger.exception(e)
                    logger.info("plugin \"%s\" failed to load.",
                                plugin_priored[i]["name"][:plugin_priored[i]["name"].find(".py")])
                    continue
            logger.info("plugin \"%s\" loaded successfully.",
                        plugin_priored[i]["name"][:plugin_priored[i]["name"].find(".py")])
